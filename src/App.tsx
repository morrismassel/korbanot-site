...
const TR = {
  ...
  gender_female_note:{en:"Female users are exempt from re'iyah, chagigah, and shalmei simcha. Machatzis hashekel is voluntary, not obligatory. Yoledes defaults to 1.",he:"נשים פטורות מעולת ראייה, חגיגה ושלמי שמחה. מחצית השקל רשות, לא חובה. יולדת: ברירת מחדל 1.",es:"Las mujeres están exentas de re'iyah, chagigah y shalmei simcha. Machatzit hashekel es voluntaria. Yoledes: valor predeterminado 1."},
...
const LIFE_IDS       = ["yoledes","olah_vol","shelamim_vol","nazir_vol","metzora_vol","oleh_yored","pidyon_haben","pesach_sheni"];
...
const FINANCIAL_TIERS = {
  poor:    {id:"poor",    label:"Poor (Ani)",     hebrew:"עָנִי",     desc:"Bird substitutions for olas re'iyah, chagigah, and yoledes.",desc_he:"תחליפי עוף לעולת ראייה, לחגיגה ולקרבן יולדת",desc_es:"Sustituciones de ave para olas re'iyah y chagigah",  reiyahId:"olah_bird",  chagigahId:"olah_bird",  yoledesId:"yoledes_poor"},
  average: {id:"average", label:"Average",        hebrew:"בֵּינוֹנִי", desc:"Standard animals. Lamb for re'iyah, ram for chagigah.",desc_he:"בהמות רגילות. כבש לראייה, איל לחגיגה",desc_es:"Animales estándar. Cordero para re'iyah, carnero para chagigah",reiyahId:"reiyah",    chagigahId:"chagigah",  yoledesId:"yoledes"},
  wealthy: {id:"wealthy", label:"Wealthy (Ashir)",hebrew:"עָשִׁיר",   desc:"Premium animals. Ram for re'iyah, bull for chagigah.",desc_he:"בהמות משובחות. איל לראייה, שור לחגיגה",desc_es:"Animales premium. Carnero para re'iyah, toro para chagigah",reiyahId:"reiyah_ram",chagigahId:"chagigah_bull",yoledesId:"yoledes"},
};
const TRAVEL_ITEMS = [
...
export default function korbanosCalculator() {
  ...
  const resolveCatalogId = id=>{
    ...
    if(id==="yoledes")  return isFemale ? (tier.yoledesId||"yoledes") : null;
  ...
  const getQty=id=>{
    ...
    if(id==="yoledes")       { if(!isFemale) return 0; const stored=profileQtys[id]; return stored!=null?stored:1; }
